...
interface WifeTransferMonthNavProps {
  year: number
  month: number
}
...
export function WifeTransferMonthNav({ year, month }: WifeTransferMonthNavProps) {
  const router = useRouter()

  function navigateTo(y: number, m: number) {
    const params = new URLSearchParams({ month: toMonthParam(y, m) })
    router.push(`/transfers/wife?${params.toString()}`)
  }
