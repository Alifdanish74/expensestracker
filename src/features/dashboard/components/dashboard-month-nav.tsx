...
/**
 * Month navigator for the Dashboard.
 * Routes to /dashboard?month=YYYY-MM.
 * Unlike the transactions MonthNav, this allows future month navigation
 * so users can pre-check upcoming months.
 */
export function DashboardMonthNav({ year, month }: DashboardMonthNavProps) {
  const router = useRouter()

  function navigateTo(y: number, m: number) {
    router.push(`/dashboard?month=${toMonthParam(y, m)}`)
  }
